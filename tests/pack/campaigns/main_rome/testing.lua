local function run_twdll_tests()
    ...
    if type(twdll) == "table" and type(twdll.Log) == "function" then
        ...
        local char = consul._game():model():world():faction_by_key('rom_rome'):faction_leader()
        ...
        twdll.Log("[TEST] Unit Initial State - Men: " .. tostring(initial_men) .. "/" .. tostring(max_men) .. " (" .. tostring(initial_percentage) .. "%)")
        ...
        twdll.Log("[TEST] Unit Modified State - Men: " .. tostring(new_men) .. "/" .. tostring(max_men) .. " (" .. tostring(new_percentage) .. "%)")
...
local _, err = pcall(function()
    table.insert(events.FirstTickAfterWorldCreated, run_twdll_tests)
end)
