...
bool PatchNewMemmoveBranch() {
    ...
    g_logFile << "  -> Target JNB is at:  " << (void *)pTargetJnb << " (Offset: 0x" << std::hex
              << offset_to_jnb << ")" << std::dec << std::endl;

    // --- VERIFICATION STEP 1: READ THE ORIGINAL BYTE ---
    unsigned char original_byte = *(unsigned char *)pTargetJnb;
    g_logFile << "  -> VERIFY: Original byte at target is 0x" << std::hex << (int)original_byte
              << std::dec << "." << std::endl;

    // Check if it's what we expect (jnb short has opcode 0x73)
    if (original_byte != 0x73) {
        g_logFile << "  [!] WARNING: Original byte was not 0x73 (jnb). Patch may be incorrect for "
    ...
    *(unsigned char *)pTargetJnb = patch;
    ...
    g_logFile << "  -> VERIFY: New byte after patch is 0x" << std::hex << (int)new_byte << std::dec
              << "." << std::endl;

    if (new_byte == patch) {
        g_logFile
            << "  -> SUCCESS: Patch confirmed. The 'rep movsb' instruction will now be bypassed."
            << std::endl;
        return true;
    } else {
        g_logFile << "  [!] FATAL: Patch verification FAILED. Wrote 0xEB but read back 0x"
                  << std::hex << (int)new_byte << "." << std::endl;
        return false;
    }
}
// =================================================================================
...
DWORD WINAPI InstallHooks(HMODULE hGameDll) {
    ...
    Hook hooks_to_install[] = {
        {"_memmove", 0x116BD8D0, (void *)&memmove},
        {"_memmove_0", 0x116BDE50, (void *)&memmove},
