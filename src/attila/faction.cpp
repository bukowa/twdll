...
Status enum values:
- `0` = `RESEARCHED` (fully unlocked and active)
- `1` = `RESEARCHED_BUT_DISABLED` (researched, but effect bundle disabled)
- `2` = `BEING_RESEARCHED` (currently selected in faction research queue)
- `3` = `AVAILABLE` (all prerequisites met, eligible to be researched)
- `4` = `UNAVAILABLE` (prerequisites missing / locked in technology tree)
- `5` = `NOT_PRESENT` (technology node not present in faction's tree)
- `6` = `LOCKED_FACTION_LEVEL` (locked by faction imperium / tier level)
...
Status enum values:
- `0` = `RESEARCHED` (instantly unlocks and applies all faction effects)
- `1` = `RESEARCHED_BUT_DISABLED` (marks researched, but disables faction bonuses)
- `2` = `BEING_RESEARCHED` (sets as active research target)
- `3` = `AVAILABLE` (makes available to be clicked and researched)
- `4` = `UNAVAILABLE` (locks / disables from research)
- `5` = `NOT_PRESENT` (marks unassigned)
- `6` = `LOCKED_FACTION_LEVEL` (locks under faction level requirements)
